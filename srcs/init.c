...
/*   Updated: 2021/08/27 18:14:05 by chly-huc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_init_info(t_philo *philo, char **argv)
{
	...
	i = -1;
	while (++i < ft_atoli(argv[1]))
	{
		philo[i].nbr = ft_atoli(argv[1]);
		philo[i].die = ft_atoli(argv[2]);
		philo[i].eat = ft_atoli(argv[3]);
		philo[i].sleep = ft_atoli(argv[4]);
		if (argv[5])
			philo[i].max_eat = ft_atoli(argv[5]);
		else
			philo[i].max_eat = -1;
		philo[i].id = i + 1;
		philo[i].past_time = 0;
